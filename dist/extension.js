  ...
  "node_modules/jsonc-parser/lib/umd/main.js"(exports2, module2) {
    (function(factory2) {
      if (typeof module2 === "object" && typeof module2.exports === "object") {
        ...
        define(["require", "exports", "./impl/format", "./impl/edit", "./impl/scanner", "./impl/parser"], factory2);
      }
      ...
      function modify2(text, path3, value, options) {
        return edit.setProperty(text, path3, value, options);
      }
      exports3.modify = modify2;
      ...
      exports3.applyEdits = applyEdits2;
    });
  }
});
...
// src/services/smitheryClient.ts
...
var SMITHERY_API_BASE = "https://registry.smithery.ai";
...
var SmitheryClient = class {
  ...
  async searchServers(query) {
    ...
    return servers.filter((server) => {
      ...
      return name.includes(lowerQuery) || displayName.includes(lowerQuery) || description.includes(lowerQuery) || tags.some((tag) => (tag || "").toLowerCase().includes(lowerQuery)) || category.includes(lowerQuery);
    });
  }
  /**
  ...
  async getCategories() {
    ...
    return Array.from(categoryCounts.entries()).map(([category, count]) => ({ category, count })).sort((a, b) => b.count - a.count);
  }
  /**
   * Get cached servers
   */
  async getCachedServers() {
    try {
      ...
      if (age < CACHE_TTL) {
        const ageMinutes = Math.round(age / 1e3 / 60);
   ...
   * Set cached servers
   */
  async setCachedServers(servers) {
    try {
      const cachePath = this.getCachePath();
      const cache = {
        servers,
...
// src/providers/mcpTreeProvider.ts
var vscode2 = __toESM(require("vscode"));
var MCPTreeItem = class extends vscode2.TreeItem {
  constructor(label, collapsibleState, server, contextValue) {
...
`);
  ...
  getIconPath(server) {
    ...
    return new vscode2.ThemeIcon(iconName);
  }
};
var MCPTreeProvider = class {
  constructor(context, smitheryClient) {
    this.context = context;
  ...
  getRootItems() {
    ...
    items.push(
      new MCPTreeItem(
        `\u{1F4C2} All Servers (${this.servers.length})`,
  ...
  getCategoryIcon(category) {
    const iconMap = {
      "databases": "\u{1F4BE}",
      "cloud-services": "\u2601\uFE0F",
      "developer-tools": "\u{1F527}",
      "analytics": "\u{1F4CA}",
      "communication": "\u{1F4AC}",
      "file-storage": "\u{1F4C1}",
      "ai-ml": "\u{1F916}",
      "productivity": "\u2705"
    };
...
var jsonc = __toESM(require_main());
...
var ConfigManager = class {
  ...
  async detectConfigLocations() {
    ...
    if (workspaceFolders && workspaceFolders.length > 0) {
      const claudeCodePath = path2.join(
   ...
   */
  async getPrimaryConfigLocation() {
  ...
  getVSCodeConfigPath() {
    ...
    if (platform === "darwin") {
      ...
      return path2.join(process.env.APPDATA || "", "Code", "User", "mcp.json");
    } else {
   ...
   */
  async addMCPServer(serverName, config, configLocation) {
    ...
    const content = await fs2.readFile(location.configPath, "utf-8");
    ...
    await fs2.writeFile(location.configPath, newContent, "utf-8");
    console.log(`[Clean] Successfully added "${serverName}" to config`);
  }
  /**
   * Remove MCP server from config file
   */
  async removeMCPServer(serverName, configLocation) {
    const location = configLocation || await this.getPrimaryConfigLocation();
    if (!location.exists) {
   ...
   */
  async serverExists(serverName, configLocation) {
    const location = configLocation || await this.getPrimaryConfigLocation();
    if (!location.exists) {
      return false;
    }
    try {
      ...
      return config?.mcpServers?.[serverName] !== void 0;
    } catch {
      return false;
    }
  }
   ...
   */
  async getInstalledServers(configLocation) {
    const location = configLocation || await this.getPrimaryConfigLocation();
    if (!location.exists) {
      return [];
    }
   ...
   */
  async createConfigFile(configPath) {
    const defaultConfig = {
      mcpServers: {}
    };
  ...
  async restoreBackup(configPath) {
    const backupPath = `${configPath}.backup`;
    try {
      await fs2.copyFile(backupPath, configPath);
...
var MCPInstaller = class {
   ...
   */
  async confirmInstallation(server, aiTool) {
...
Command: ${this.getCommand(server)} ${this.getArgs(server).join(" ")}`;
   ...
   */
  async trackInstallation(serverName) {
    const installed = this.context.globalState.get("installedServers", []);
   ...
   */
  async removeFromTracking(serverName) {
    const installed = this.context.globalState.get("installedServers", []);
...
async function activate(context) {
  ...
  const treeView = vscode5.window.createTreeView("mcpServers", {
  ...
  const statusBarItem = vscode5.window.createStatusBarItem(
  ...
  vscode5.window.showInformationMessage("Clean MCP Manager activated! \u{1F9F9}");
}
function registerCommands(context, smitheryClient, treeProvider, mcpInstaller) {
  context.subscriptions.push(
    vscode5.commands.registerCommand("clean.refreshServers", async () => {
      try {
        await smitheryClient.clearCache();
        await treeProvider.initialize();
        vscode5.window.showInformationMessage("MCP servers refreshed! \u{1F504}");
      } catch (error) {
  ...
  context.subscriptions.push(
    vscode5.commands.registerCommand("clean.browseMCPs", async () => {
      vscode5.window.showInformationMessage("MCP Browser (coming soon!)");
    })
  );
  context.subscriptions.push(
    vscode5.commands.registerCommand("clean.installMCP", async (server) => {
      if (!server || !server.server) {
        vscode5.window.showErrorMessage("No server selected for installation");
        return;
      }
      try {
        const result = await mcpInstaller.installServer(server.server);
        if (result.success) {
          vscode5.window.showInformationMessage(result.message);
          if (result.requiresReload) {
            await mcpInstaller.promptReload(server.server.displayName);
          }
          await treeProvider.initialize();
        } else {
          vscode5.window.showErrorMessage(result.message);
        }
      } catch (error) {
        vscode5.window.showErrorMessage(
  ...
  context.subscriptions.push(
    vscode5.commands.registerCommand("clean.showServerDetails", async (server) => {
      if (server) {
  ...
  context.subscriptions.push(
    vscode5.commands.registerCommand("clean.showStats", async () => {
